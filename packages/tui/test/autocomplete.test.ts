...
describe("CombinedAutocompleteProvider", () => {
	...
	describe("fd @ file suggestions", { skip: !isFdInstalled }, () => {
		...
		beforeEach(() => {
			rootDir = mkdtempSync(join(tmpdir(), "deepseek-helmsman-autocomplete-root-"));
		...
		test("includes hidden paths but excludes .git", async () => {
			setupFolder(baseDir, {
				dirs: [".deepseek-helmsman", ".github", ".git"],
				files: {
					".deepseek-helmsman/config.json": "{}",
			...
			const values = result?.items.map((item) => item.value) ?? [];
			assert.ok(values.includes("@.deepseek-helmsman/"));
	...
	describe("dot-slash path completion", () => {
		...
		beforeEach(() => {
			baseDir = mkdtempSync(join(tmpdir(), "deepseek-helmsman-autocomplete-"));
		});
