...
DeepSeek Helmsman saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.deepseek-helmsman/agent/sessions/`, organized by working directory. Each session is a JSONL file with a tree structure.

```bash
deepseek-helmsman -c                  # Continue most recent session
deepseek-helmsman -r                  # Browse and select from past sessions
...
deepseek-helmsman --session <path|id> # Use a specific session file or partial session ID
deepseek-helmsman --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `deepseek-helmsman -r` opens the same picker at startup.
...
When available, DeepSeek Helmsman uses the `trash` CLI for deletion instead of permanently removing files.
...
```bash
deepseek-helmsman --name "Refactor auth module"
deepseek-helmsman --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `deepseek-helmsman -r`.
...
When `/tree` switches away from one branch to another, DeepSeek Helmsman can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
