> DeepSeek Helmsman can create themes. Ask it to build one for your setup.
...
DeepSeek Helmsman loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.deepseek-helmsman/agent/themes/*.json`
- Project: `.deepseek-helmsman/themes/*.json`
- Packages: `themes/` directories or package manifest entries in `package.json`
...
On first run, DeepSeek Helmsman detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.deepseek-helmsman/agent/themes
vim ~/.deepseek-helmsman/agent/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, DeepSeek Helmsman reloads it automatically for immediate visual feedback.
...
DeepSeek Helmsman uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, it falls back to the nearest approximation.
