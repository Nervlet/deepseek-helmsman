> DeepSeek Helmsman can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** `@deepseek-helmsman/tui`
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `DEEPSEEK_HELMSMAN_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
Import from `@deepseek-helmsman/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@deepseek-helmsman/tui";
```
...
```typescript
import { getMarkdownTheme } from "@deepseek-helmsman/coding-agent";
import { Markdown } from "@deepseek-helmsman/tui";
...
For letting users pick from a list of options. Use `SelectList` from `@deepseek-helmsman/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@deepseek-helmsman/coding-agent";
import { DynamicBorder } from "@deepseek-helmsman/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@deepseek-helmsman/tui";
...
```typescript
import { BorderedLoader } from "@deepseek-helmsman/coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@deepseek-helmsman/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@deepseek-helmsman/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@deepseek-helmsman/tui";
...
Customize the inline working indicator shown while DeepSeek Helmsman is streaming a response.
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@deepseek-helmsman/coding-agent";
import { matchesKey, truncateToWidth } from "@deepseek-helmsman/tui";
