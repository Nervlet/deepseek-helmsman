...
type SkillDiscoveryMode = "helmsman" | "agents";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "helmsman" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function readDeepSeekHelmsmanManifestFile(packageJsonPath: string): DeepSeekHelmsmanManifest | null {
	try {
		const content = readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content) as { deepseekHelmsman?: DeepSeekHelmsmanManifest };
		return pkg.deepseekHelmsman ?? null;
	} catch {
		return null;
	}
}
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (existsSync(packageJsonPath)) {
		const manifest = readDeepSeekHelmsmanManifestFile(packageJsonPath);
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "helmsman");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		const packageManagerName = this.getPackageManagerName();
		// Extension packages run inside DeepSeek Helmsman and resolve bundled APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// @deepseek-helmsman/* peers. Stale auto-installed peers can otherwise block updates.
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "deepseek-helmsman-extensions", private: true };
	...
	private getTemporaryDir(prefix: string, suffix?: string): string {
		const hash = createHash("sha256")
			...
			.digest("hex")
			.slice(0, 8);
		return join(tmpdir(), "deepseek-helmsman-extensions", prefix, hash, suffix ?? "");
	}
	...
	private collectPackageResources(
		...
		const manifest = this.readDeepSeekHelmsmanManifest(packageRoot);
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof DeepSeekHelmsmanManifest];
	...
	private readDeepSeekHelmsmanManifest(packageRoot: string): DeepSeekHelmsmanManifest | null {
	...
	private addAutoDiscoveredResources(
		...
		// Project extensions from the project config directory
		addResources(
			"extensions",
		...
		// Project skills from the project config directory
		addResources(
			"skills",
			collectAutoSkillEntries(projectDirs.skills, "helmsman"),
			projectMetadata,
		...
		// User extensions from the agent config directory
		addResources(
			"extensions",
		...
		// User skills from the agent config directory
		addResources(
			"skills",
			collectAutoSkillEntries(userDirs.skills, "helmsman"),
			userMetadata,
