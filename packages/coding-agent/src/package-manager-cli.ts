import { Markdown, type MarkdownTheme } from "@deepseek-helmsman/tui";
...
import { getLatestDeepSeekHelmsmanRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
Options:
  -l, --local    Install project-locally (.deepseek-helmsman/settings.json)
...
Options:
  -l, --local    Remove from project settings (.deepseek-helmsman/settings.json)
...
Update DeepSeek Helmsman and installed packages.

Options:
  --self                  Update DeepSeek Helmsman only
  --extensions            Update installed packages only
  --extension <source>    Update one package only
  --force                 Reinstall DeepSeek Helmsman even if the current version is latest

Short forms:
  ${APP_NAME} update                Update DeepSeek Helmsman and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update self           Update DeepSeek Helmsman only
`);
			return;
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	...
	try {
		const latestRelease = await getLatestDeepSeekHelmsmanRelease(VERSION);
			...
			case "update": {
				...
				if (updateTargetIncludesSelf(target)) {
					...
					if (
						...
						console.error(
							chalk.red(`${APP_NAME} self-update on Windows is only supported for Bun, npm, and pnpm installs.`),
						);
