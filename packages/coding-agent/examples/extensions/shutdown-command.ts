 ...
 * Demonstrates how extensions can use ctx.shutdown() to exit DeepSeek Helmsman cleanly.
 */
...
export default function (api: ExtensionAPI) {
	// Register a /quit command that cleanly exits DeepSeek Helmsman
	api.registerCommand("quit", {
		description: "Exit DeepSeek Helmsman cleanly",
	...
	api.registerTool({
		...
		description: "Complete a task and exit DeepSeek Helmsman",
	...
	api.registerTool({
		...
		description: "Deploy the application and exit DeepSeek Helmsman",
		parameters: Type.Object({
		...
		async execute(_toolCallId, params, _signal, onUpdate, ctx) {
			...
			// const result = await api.exec("bun", ["run", "deploy", params.environment], { signal });
