 ...
 *
 * Usage: deepseek-helmsman --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@deepseek-helmsman/coding-agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@deepseek-helmsman/tui";
import { matchesKey, truncateToWidth, visibleWidth } from "@deepseek-helmsman/tui";
...
export default function (api: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates for game-like rendering.
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/hanbing/Nervlet/deepseek-helmsman/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/hanbing/Nervlet/deepseek-helmsman\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " Overlay games use the same approach.")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
