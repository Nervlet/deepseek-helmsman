 ...
 * (logo + keybinding hints) with a custom header component.
 */

import type { ExtensionAPI, Theme } from "@deepseek-helmsman/coding-agent";
import { VERSION } from "@deepseek-helmsman/coding-agent";

// --- HELMSMAN MASCOT ---
// Based on the built-in agent header character.
function getHelmsmanMascot(theme: Theme): string[] {
	// --- COLORS ---
	// 3b1b Blue: R=80, G=180, B=230
	const helmsmanBlue = (text: string) => theme.fg("accent", text);
	...
	const lineBar = `  ${helmsmanBlue(BLOCK.repeat(14))}`;

	// 4. Lines 3-6: The Legs
	// Indented 5 spaces relative to the very left edge
	// Leg width: 2 blocks | Gap: 4 blocks
	const lineLeg = `     ${helmsmanBlue(BLOCK.repeat(2))}    ${helmsmanBlue(BLOCK.repeat(2))}`;
			...
			ctx.ui.setHeader((_tui, theme) => {
				return {
					render(_width: number): string[] {
						const mascotLines = getHelmsmanMascot(theme);
