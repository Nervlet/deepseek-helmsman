 ...
 * working indicator shown while DeepSeek Helmsman is streaming a response.
 *
 * Usage:
 *   deepseek-helmsman --extension examples/extensions/working-indicator.ts
 *
 * Commands:
 ...
 *   /working-indicator reset     Restore DeepSeek Helmsman's default spinner
 */

import type { ExtensionAPI, ExtensionContext, WorkingIndicatorOptions } from "@deepseek-helmsman/coding-agent";
...
function describeMode(mode: WorkingIndicatorMode): string {
	switch (mode) {
		...
		case "default":
			return "DeepSeek Helmsman default spinner";
	}
}
