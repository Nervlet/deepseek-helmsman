...
describe("theme picker", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "deepseek-helmsman-theme-picker-"));
		const agentDir = join(tempRoot, "agent");
		vi.stubEnv("DEEPSEEK_HELMSMAN_CODING_AGENT_DIR", agentDir);
	...
	it("uses custom theme content names instead of file names", () => {
		...
		const themePath = join(process.env.DEEPSEEK_HELMSMAN_CODING_AGENT_DIR!, "themes", "foo.json");
