...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.DEEPSEEK_HELMSMAN_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.DEEPSEEK_HELMSMAN_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .deepseek-helmsman skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .deepseek-helmsman", async () => {
			const extDir = join(tempDir, ".deepseek-helmsman", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".deepseek-helmsman"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".deepseek-helmsman", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".deepseek-helmsman", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".deepseek-helmsman", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".deepseek-helmsman", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".deepseek-helmsman", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json deepseekHelmsman.extensions in extensions setting", async () => {
			// Create a package with deepseekHelmsman.extensions in package.json
			...
			// Should find the extensions declared in package.json deepseekHelmsman.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .deepseek-helmsman/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-helmsman", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-helmsman"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-helmsman\ndescription: user helmsman\n---\n");
		...
		it("should use the project .deepseek-helmsman dir as baseDir for project .deepseek-helmsman skills", async () => {
			const projectBaseDir = join(tempDir, ".deepseek-helmsman");
			const skillPath = join(projectBaseDir, "skills", "project-helmsman", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-helmsman"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-helmsman\ndescription: project helmsman\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".deepseek-helmsman", "agent");
		...
		it("should dedupe user skill entries when ~/.deepseek-helmsman/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .deepseek-helmsman auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".deepseek-helmsman\n");

			const skillDir = join(tempDir, ".deepseek-helmsman", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with deepseekHelmsman manifest", async () => {
		...
		it("should keep deepseekHelmsman manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.deepseek-helmsman\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".deepseek-helmsman", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the deepseek-helmsman-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .deepseek-helmsman settings base", () => {
			...
			const rel = relative(join(tempDir, ".deepseek-helmsman"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in deepseekHelmsman manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json deepseekHelmsman.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".deepseek-helmsman", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".deepseek-helmsman", "npm", "node_modules", "project-current");
		...
		it("should skip installing missing package sources when offline", async () => {
			process.env.DEEPSEEK_HELMSMAN_OFFLINE = "1";
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".deepseek-helmsman", "npm", "node_modules", "example");
