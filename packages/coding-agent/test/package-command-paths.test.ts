...
describe("package commands", () => {
	...
	beforeEach(() => {
		tempDir = join(
			tmpdir(),
			`deepseek-helmsman-package-commands-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
		...
		originalPackageDir = process.env.DEEPSEEK_HELMSMAN_PACKAGE_DIR;
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("deepseek-helmsman install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "deepseek-helmsman --help" or "deepseek-helmsman install <source> [-l]".');
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: deepseek-helmsman install <source> [-l]");
	...
	it("uses global npmCommand and current package name for forced self updates without checking the api", async () => {
		...
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@deepseek-helmsman", "coding-agent");
		const fakeNpmPath = join(tempDir, "fake-npm.cjs");
		const recordPath = join(tempDir, "self-update.json");
		mkdirSync(selfPackageDir, { recursive: true });
		mkdirSync(join(projectDir, ".deepseek-helmsman"), { recursive: true });
		writeFileSync(
			fakeNpmPath,
...
`,
		...
		writeFileSync(
			join(projectDir, ".deepseek-helmsman", "settings.json"),
			JSON.stringify({ npmCommand: [originalExecPath, fakeNpmPath, "--prefix", projectPrefix] }, null, 2),
		);
		process.env.DEEPSEEK_HELMSMAN_PACKAGE_DIR = selfPackageDir;
...
`,
		...
		const activePackageName =
			PACKAGE_NAME === "@new-scope/deepseek-helmsman"
				? "@newer-scope/deepseek-helmsman"
...
`,
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).not.toContain(`Updated deepseek-helmsman`);
	...
	it("suggests the configured source when update input omits the npm prefix", async () => {
		const settingsPath = join(agentDir, "settings.json");
		writeFileSync(settingsPath, JSON.stringify({ packages: ["npm:deepseek-helmsman-formatter"] }, null, 2));

		const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
		const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});

		try {
			await expect(main(["update", "deepseek-helmsman-formatter"])).resolves.toBeUndefined();

			const stderr = errorSpy.mock.calls.map(([message]) => String(message)).join("\n");
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stderr).toContain("Did you mean npm:deepseek-helmsman-formatter?");
			expect(stdout).not.toContain("Updated deepseek-helmsman-formatter");
			expect(process.exitCode).toBe(1);

			const settings = JSON.parse(readFileSync(settingsPath, "utf-8")) as { packages?: string[] };
			expect(settings.packages).toContain("npm:deepseek-helmsman-formatter");
		} finally {
			errorSpy.mockRestore();
			logSpy.mockRestore();
		}
