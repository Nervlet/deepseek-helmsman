...
import { Container, type Terminal, Text, TUI } from "@deepseek-helmsman/tui";
...
describe("edit tool TUI rendering", () => {
	...
	it("renders the large diff in the call preview and does not full-redraw when the result settles", async () => {
		const dir = await mkdtemp(join(tmpdir(), "deepseek-helmsman-edit-redraw-"));
		tempDirs.push(dir);
...
`,
	...
	it("reconstructs the boxed preview from a settled result without argsComplete", async () => {
		const dir = await mkdtemp(join(tmpdir(), "deepseek-helmsman-edit-replay-"));
		tempDirs.push(dir);
...
`,
	...
	it("shows a preflight error without rendering a diff when the edits do not apply", async () => {
		const dir = await mkdtemp(join(tmpdir(), "deepseek-helmsman-edit-preflight-"));
		tempDirs.push(dir);
