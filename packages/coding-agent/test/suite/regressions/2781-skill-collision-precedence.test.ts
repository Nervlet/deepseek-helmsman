...
describe("issue #2781 skill collision precedence: user skills should override package skills", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `deepseek-helmsman-2781-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	function createPackageWithSkill(name: string, description: string): string {
		...
		writeFileSync(
			...
			JSON.stringify(
				{ name: `fake-pkg-${name}`, version: "1.0.0", deepseekHelmsman: { skills: [`skills/${name}`] } },
				null,
				2,
			),
		);
	...
	function createProjectSkill(name: string, description: string): string {
		const skillDir = join(cwd, ".deepseek-helmsman", "skills", name);
		mkdirSync(skillDir, { recursive: true });
		const skillPath = join(skillDir, "SKILL.md");
	...
	function createSettingsWithPackage(pkgDir: string, scope: "user" | "project"): void {
		const settingsDir = scope === "user" ? agentDir : join(cwd, ".deepseek-helmsman");
