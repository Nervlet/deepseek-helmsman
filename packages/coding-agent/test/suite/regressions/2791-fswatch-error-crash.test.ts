...
/**
 * Regression test for https://github.com/hanbing/deepseek-helmsman/issues/2791
 *
...
describe("issue #2791 fs.watch error event crashes process", () => {
	...
	beforeEach(() => {
		tempRoot = mkdtempSync(join(tmpdir(), "deepseek-helmsman-2791-"));
...
process.env.DEEPSEEK_HELMSMAN_CODING_AGENT_DIR = "${agentDir}";
...
`,
		...
		try {
			_stdout = execFileSync(process.execPath, [scriptPath], {
				timeout: 10000,
				encoding: "utf-8",
				env: { ...process.env, DEEPSEEK_HELMSMAN_CODING_AGENT_DIR: agentDir },
