...
// Regression for https://github.com/hanbing/deepseek-helmsman/issues/5080
//
...
function createSessionManager(options: { sessionFile?: string } = {}): SessionManager {
	return {
		...
		getSessionDir: () => "/tmp/deepseek-helmsman-sessions",
		usesDefaultSessionDir: () => true,
	} as unknown as SessionManager;
}

function createTempFile(): string {
	const dir = mkdtempSync(join(tmpdir(), "deepseek-helmsman-shutdown-resume-hint-"));
	tempDirs.push(dir);
