...
import { fauxAssistantMessage, fauxToolCall } from "@deepseek-helmsman/ai";
...
async function createWaitingHarness(
	options: {
		...
		extensionFactories?: Harness["session"]["extensionRunner"] extends never
			? never
			: Array<(extensionApi: ExtensionAPI) => void>;
	} = {},
): Promise<{
	harness: Harness;
...
describe("AgentSession queue characterization", () => {
	...
	it("dispatches extension commands immediately when prompted while idle", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(extensionApi) => {
					extensionApi.registerCommand("testcmd", {
			...
			extensionFactories: [
				(extensionApi: ExtensionAPI) => {
					extensionApi.on("agent_end", async () => {
						if (sent) return;
						sent = true;
						extensionApi.sendUserMessage("conflict report", { deliverAs: "followUp" });
					});
				},
			],
		});
