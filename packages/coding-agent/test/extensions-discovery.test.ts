...
describe("extensions discovery", () => {
	...
	beforeEach(() => {
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "deepseek-helmsman-ext-test-"));
	...
	const extensionCode = `
		export default function(extensionApi) {
			extensionApi.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("discovers subdirectory with package.json deepseekHelmsman field", async () => {
	...
	it("keeps package.json deepseekHelmsman extension entries with leading tilde package-relative", async () => {
	...
	it("package.json with deepseekHelmsman field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without deepseekHelmsman field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
	...
	it("handles mixed direct files and subdirectories", async () => {
		...
		fs.writeFileSync(
			...
			JSON.stringify({ deepseekHelmsman: { extensions: ["./entry.ts"] } }),
		);
	...
	it("registers message renderers", async () => {
		const extCode = `
			export default function(extensionApi) {
				extensionApi.registerMessageRenderer("my-custom-type", (message, options, theme) => {
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(extensionApi) {
				extensionApi.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(extensionApi) {
				...
				extensionApi.on("tool_call", async (event) => undefined);
				extensionApi.on("agent_end", async () => {});
			}
		`;
	...
	it("loads extension with shortcuts", async () => {
		const extCode = `
			export default function(extensionApi) {
				extensionApi.registerShortcut("ctrl+t", {
