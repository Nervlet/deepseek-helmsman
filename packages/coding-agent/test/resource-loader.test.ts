...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "deepseek-helmsman-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".deepseek-helmsman", "prompts");
			...
			const projectSkillDir = join(cwd, ".deepseek-helmsman", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".deepseek-helmsman", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".deepseek-helmsman", "themes"), { recursive: true });
	...
	extensionApi.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".deepseek-helmsman"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".deepseek-helmsman", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".deepseek-helmsman", "extensions", "shared.ts"));
		});

		it("should keep both extensions loaded when command names collide", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".deepseek-helmsman", "extensions");
			...
			writeFileSync(
				...
				`export default function(extensionApi) {
	extensionApi.registerCommand("deploy", {
		description: "project deploy",
		handler: async () => {},
	});
	extensionApi.registerCommand("project-only", {
	...
	extensionApi.registerCommand("user-only", {
...
Content`,
		...
		it("should skip AGENTS.md and CLAUDE.md discovery when noContextFiles is true", async () => {
			writeFileSync(join(cwd, "AGENTS.md"), "# Project Guidelines\n\nBe helpful.");
			writeFileSync(join(cwd, "CLAUDE.md"), "# Compatibility Guidelines\n\nBe helpful.");
		...
		it("should discover SYSTEM.md from cwd/.deepseek-helmsman", async () => {
			const configDir = join(cwd, ".deepseek-helmsman");
