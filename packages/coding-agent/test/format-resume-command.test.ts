...
function createTempFile(): string {
	const dir = mkdtempSync(join(tmpdir(), "deepseek-helmsman-format-resume-command-"));
	tempDirs.push(dir);
...
function createSessionManager(options: {
	...
	return {
		...
		getSessionDir: () => options.sessionDir ?? "/tmp/deepseek-helmsman-sessions",
...
describe("formatResumeCommand", () => {
	...
	it("includes unquoted safe session dirs for non-default session dirs", () => {
		...
		const sessionManager = createSessionManager({
			...
			sessionDir: "/tmp/custom-deepseek-helmsman-sessions",
			usesDefaultSessionDir: false,
		});

		expect(formatResumeCommand(sessionManager)).toBe(
			`${APP_NAME} --session-dir /tmp/custom-deepseek-helmsman-sessions --session test-session`,
		);
	});
	...
	it("quotes session dirs containing single quotes", () => {
		...
		const sessionManager = createSessionManager({
			sessionFile,
			sessionId: "test-session",
			sessionDir: "/tmp/custom deepseek-helmsman's sessions",
			usesDefaultSessionDir: false,
		});

		expect(formatResumeCommand(sessionManager)).toBe(
			`${APP_NAME} --session-dir '/tmp/custom deepseek-helmsman'\\''s sessions' --session test-session`,
		);
	});
	...
	it("returns undefined when the session file is missing", () => {
		setStdoutIsTTY(true);
		const sessionManager = createSessionManager({ sessionFile: "/tmp/deepseek-helmsman-missing-session.jsonl" });
