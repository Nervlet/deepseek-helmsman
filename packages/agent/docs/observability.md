...
# DeepSeek Helmsman Observability Design Notes

## Goal

Make `packages/ai` and `packages/agent`/harness observable without depending on OpenTelemetry, Sentry, or any APM vendor.

DeepSeek Helmsman should emit stable, structured lifecycle events. External listeners can convert those events into OTel spans, Sentry spans, logs, metrics, or custom telemetry.
...
```text
traceId=t1 spanId=s1 parent=-  name=deepseek_helmsman.agent.prompt
traceId=t1 spanId=s2 parent=s1 name=deepseek_helmsman.agent.turn
traceId=t1 spanId=s3 parent=s2 name=deepseek_helmsman.ai.provider.request
traceId=t1 spanId=s4 parent=s2 name=deepseek_helmsman.agent.tool_call
traceId=t1 spanId=s5 parent=s4 name=deepseek_helmsman.session.append_entry
```
...
```ts
await Promise.all([
  runWithHelmsmanContext({ userId: "alice" }, () => harness.prompt("A")),
  runWithHelmsmanContext({ userId: "bob" }, () => harness.prompt("B")),
]);
```

Deep code can then read the correct current context for the active async chain.

DeepSeek Helmsman must run in Node, Bun, browser, workers, and other JS runtimes, so ALS cannot be the core abstraction. It should be a runtime adapter.

## Core design

DeepSeek Helmsman owns a small runtime-agnostic observability abstraction:

```ts
export interface HelmsmanObservabilityContext {
  traceId?: string;
  currentSpanId?: string;
  userContext?: Record<string, unknown>;
}

export interface HelmsmanObservabilityEvent {
...
export interface HelmsmanObservability {
  getContext(): HelmsmanObservabilityContext | undefined;
  runWithContext<T>(context: HelmsmanObservabilityContext, fn: () => T): T;
  emit(event: HelmsmanObservabilityEvent): void;
...
```ts
export function configureHelmsmanObservability(observability: HelmsmanObservability): void;
export function subscribeHelmsmanObservability(listener: (event: HelmsmanObservabilityEvent) => void): () => void;
export function runWithHelmsmanContext<T>(userContext: Record<string, unknown>, fn: () => T): T;
...
channel("deepseek_helmsman.observability").publish(event);
```

Subscribers can create OTel/Sentry spans without monkey-patching DeepSeek Helmsman.

## What DeepSeek Helmsman emits

DeepSeek Helmsman emits what happened. It does not create OTel/Sentry spans directly.

Initial minimal event names:

```text
deepseek_helmsman.agent.prompt
deepseek_helmsman.agent.skill
deepseek_helmsman.agent.prompt_template
deepseek_helmsman.agent.compaction
deepseek_helmsman.agent.branch_navigation
deepseek_helmsman.agent.session.append_entry
deepseek_helmsman.ai.provider.request
```
...
```text
deepseek_helmsman.agent.turn
deepseek_helmsman.agent.tool_call
deepseek_helmsman.agent.queue_update
deepseek_helmsman.ai.provider.retry
deepseek_helmsman.ai.provider.first_token
deepseek_helmsman.ai.provider.usage
deepseek_helmsman.session.read
deepseek_helmsman.session.write
```
...
Observability must never affect DeepSeek Helmsman execution.
...
{
  type: "start",
  name: "deepseek_helmsman.ai.provider.request",
  traceId: "t1",
  spanId: "s3",
  parentSpanId: "s1",
  context: {
...
```text
packages/ai
  emits deepseek_helmsman.ai.* events

packages/agent
  emits deepseek_helmsman.agent.* / deepseek_helmsman.session.* events
```
...
packages/otel
  subscribes to DeepSeek Helmsman events and creates OpenTelemetry spans
```

## Thesis

DeepSeek Helmsman defines a stable, safe event contract. Adapters define where events go.
