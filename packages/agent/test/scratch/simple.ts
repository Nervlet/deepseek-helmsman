...
const { skills: sourcedSkills } = await loadSourcedSkills<Source, SourcedSkill>(
	env,
	[
		source("project", join(env.cwd, ".deepseek-helmsman/skills")),
		source("user", join(homedir(), ".deepseek-helmsman/agent/skills")),
		source("path", join(env.cwd, "../../../deepseek-helmsman-skills")),
	],
...
const { promptTemplates: sourcedPromptTemplates } = await loadSourcedPromptTemplates<Source, SourcedPromptTemplate>(
	env,
	[
		source("project", join(env.cwd, ".deepseek-helmsman/prompts")),
		source("user", join(homedir(), ".deepseek-helmsman/agent/prompts")),
	],
