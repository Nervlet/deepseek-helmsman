...
const agentDirEnvName = "DEEPSEEK_HELMSMAN_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "DEEPSEEK_HELMSMAN_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific DEEPSEEK_HELMSMAN_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force DEEPSEEK_HELMSMAN_OFFLINE=1 / DEEPSEEK_HELMSMAN_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.DEEPSEEK_HELMSMAN_OFFLINE = "1";
		env.DEEPSEEK_HELMSMAN_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir
		? mkdtempSync(join(tmpdir(), "deepseek-helmsman-startup-benchmark-"))
		: undefined;
