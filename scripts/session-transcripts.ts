 ...
 *   --analyze      Spawn DeepSeek Helmsman subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("deepseek-helmsman", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn DeepSeek Helmsman: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".deepseek-helmsman/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn DeepSeek Helmsman subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".deepseek-helmsman/agent/AGENTS.md");
