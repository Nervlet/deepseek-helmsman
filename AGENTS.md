...
Multiple DeepSeek Helmsman sessions may be running in this cwd at the same time, each modifying different files. Git operations that touch unstaged, staged, or untracked files outside your own changes will stomp on other sessions' work. Follow these rules:
...
## Testing DeepSeek Helmsman Interactive Mode with tmux

Run the TUI in a controlled terminal (from the repo root):

```bash
tmux new-session -d -s deepseek-helmsman-test -x 80 -y 24
tmux send-keys -t deepseek-helmsman-test "./deepseek-helmsman-test.sh" Enter
sleep 3 && tmux capture-pane -t deepseek-helmsman-test -p     # capture after startup
tmux send-keys -t deepseek-helmsman-test "your prompt here" Enter
tmux send-keys -t deepseek-helmsman-test Escape               # special keys (also C-o for ctrl+o, etc.)
tmux kill-session -t deepseek-helmsman-test
```
