# Contributing to DeepSeek Helmsman
...
If you use an agent, run it from the `deepseek-helmsman` root directory so it picks up `AGENTS.md` automatically. Your agent must follow the rules and guidelines in that file.
...
DeepSeek Helmsman's core is minimal. If your feature does not belong in the core, it should be an extension. PRs that bloat the core will likely be rejected.
